...
object GuiHelper {
    ...
    private fun generateItem(subsection: ConfigurationSection): ItemStack =
        generateItem(
            ...
            subsection.getStringList("Lore").map { line ->
                MessageUtil.parseFormattedMessage(line).decorationIfAbsent(TextDecoration.ITALIC, TextDecoration.State.FALSE)
            },
        )

    fun createButtonFromConfig(subsection: ConfigurationSection): SGButton {
        return SGButton(generateItem(subsection))
    }
    ...
    fun getFallbackButton(config: ConfigurationSection): SGButton {
        ...
        item.itemMeta = itemMeta
        item.lore(
            fallbackConfig.getStringList("Lore").map { line ->
